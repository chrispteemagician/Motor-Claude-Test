// Motor-Oid — Patreon OAuth handler
// Checks membership tier: villager (£3) / elder (£7) / founder (£15)
...
exports.handler = async (event) => {
    ...
    if (!code) {
        return { statusCode: 400, headers: CORS_HEADERS, body: JSON.stringify({ error: 'Missing code parameter' }) };
    }
    ...
    try {
        ...
        if (!tokenRes.ok) {
            ...
            return { statusCode: 401, headers: CORS_HEADERS, body: JSON.stringify({ error: 'Token exchange failed' }) };
        }

        const { access_token } = await tokenRes.json();

        // Fetch identity with memberships
        const identityUrl = `https://www.patreon.com/api/oauth2/v2/identity?include=memberships&fields[member]=patron_status,currently_entitled_amount_cents`;
        const identityRes = await fetch(identityUrl, {
            headers: { Authorization: `Bearer ${access_token}` }
        });

        if (!identityRes.ok) {
            return { statusCode: 401, headers: CORS_HEADERS, body: JSON.stringify({ error: 'Identity fetch failed' }) };
        }
